...
class MainViewModel @Inject constructor(private val currencyRepository: CurrencyRepository, private val sharPref: PreferenceHelper) : ViewModel() {
    ...
    private val _changeCurrencyLive = MutableLiveData<CurrencyType>()
    val changeCurrencyLive: LiveData<CurrencyType>
        get() = _changeCurrencyLive

    private val _newCurrencySelected = SingleLiveEvent<Boolean>()
    val newCurrencySelected: LiveData<Boolean>
        get() = _newCurrencySelected
    ...
    var changeLine: Int = 1
    lateinit var inputCurrencyEntity: CurrencyEntity
    var calculateCurrencyEntityList = mutableListOf<CurrencyEntity>()
    var recentCurrencyTypes = mutableListOf<CurrencyType>()

    init {
        recentCurrencyTypes = sharPref.restoreRecentCurrencyTypes().toMutableList()
        //recentCurrencyTypes = sharPref.restoreList(RECENT_CURRENCY_TYPE_KEY) as MutableList<CurrencyType>?
        Log.d("LOG_TAG","MainViewModel: init: recentCurrencyTypes size: ${recentCurrencyTypes.size}")
    }

    fun selectCurrency(currencyType: CurrencyType){
        Log.d("LOG_TAG","MainViewModel: selectCurrency: $currencyType, changeLine: $changeLine")

        //if(recentCurrencyTypes[0] != currencyType){}
        val currencyTypeIndex = recentCurrencyTypes.indexOf(currencyType)
        if(currencyTypeIndex == -1){
            recentCurrencyTypes.add(0, currencyType)
            if(recentCurrencyTypes.size >= 5){
                recentCurrencyTypes.removeAt(recentCurrencyTypes.size -1)
            }
            for(currencyTypeRecent in recentCurrencyTypes){
                Log.d("LOG_TAG","MainViewModel: selectCurrency: recentCurrencyType $currencyTypeRecent")
            }

            sharPref.storeRecentCurrencyTypes(recentCurrencyTypes)
        }else{
            recentCurrencyTypes.removeAt(currencyTypeIndex)
        ...
        //if(currencyType == _changeCurrencyLive.value){}

        if(changeLine == 1){
            viewModelScope.launch {
                inputCurrencyEntity = currencyRepository.localDataSource.getCurrencyEntityByCurrencyType(currencyType)
                sharPref.storeInputCurrencyType(currencyType.ordinal)
                Log.d(TAG, "MainViewModel: selectCurrency: change input currency: $inputCurrencyEntity")
            }
        }

        _changeCurrencyLive.postValue(currencyType)
        _newCurrencySelected.postValue(true)
    }
    ...
    fun changeLinesCount(lineCount: Int){

        Log.d("LOG_TAG","MainViewModel: changeLinesCount: lineCount: $lineCount, current line count: ${calculateCurrencyEntityList.size}")

        viewModelScope.launch {
            //If selected lineCount is greater than now
            if(lineCount > calculateCurrencyEntityList.size){
                val lineCountDiff = lineCount - calculateCurrencyEntityList.size
                when(lineCountDiff){
                    1 -> {
                        //TODO: Change to remembered currencyType
                        calculateCurrencyEntityList.add(currencyRepository.localDataSource.getCurrencyEntityByCurrencyType(CurrencyType.BRL))
                    }
                    2 -> {
                        calculateCurrencyEntityList.add(currencyRepository.localDataSource.getCurrencyEntityByCurrencyType(CurrencyType.INR))
                        calculateCurrencyEntityList.add(currencyRepository.localDataSource.getCurrencyEntityByCurrencyType(CurrencyType.CZK))
                    }
                }
                Log.d("LOG_TAG","MainViewModel: changeLinesCount(after): lineCountDiff: $lineCountDiff, final lineCount: ${calculateCurrencyEntityList.size}")

                val currencyLines = mutableListOf<CurrencyLine>()

                for (_currencyEntity in calculateCurrencyEntityList){
                    if(_currencyEntity.currency == _currencyEntity.baseCurrency){
                        val value = inputValue / inputCurrencyEntity.rate
                        val currencyLine = CurrencyLine(_currencyEntity, value)

                        currencyLines.add(currencyLine)
                    }else{
                        val value = inputValue / inputCurrencyEntity.rate * _currencyEntity.rate
                        val currencyLine = CurrencyLine(_currencyEntity, value)
                ...
                _watchedCurrencyTypes.postValue(currencyLines)
            } else if(lineCount < calculateCurrencyEntityList.size){
                val lineCountDiff = calculateCurrencyEntityList.size - lineCount
                when(lineCountDiff){
                    1 -> {
                        calculateCurrencyEntityList.removeAt(calculateCurrencyEntityList.size-1)
                    }
                    2 -> {
                        calculateCurrencyEntityList.removeAt(calculateCurrencyEntityList.size-1)
                        calculateCurrencyEntityList.removeAt(calculateCurrencyEntityList.size-1)
        ...
        //Типы валют которые мы используем(начиная со второй строки)
        //_watchedCurrencyTypes

        //Список валют с которыми миы сейчас работаем(начиная со второй строки)
        //calculateCurrencyEntityList
    }

    fun shuffleLines(){
        calculateCurrencyEntityList.shuffle()
    ...
    //Change currency type on line
    fun changeCalculateLine(input: Float){
        //_watchedCurrencyTypes
        viewModelScope.launch {
            //val changeCurrencyEntity = currencyRepository.localDataSource.getCurrencyEntityByCurrencyType(changeCurrencyLive.value!!)

            Log.d(TAG,"MainViewModel: changeCalculateLine:input: $input, changeLine: $changeLine, linesTo calculate: ${calculateCurrencyEntityList.size}")

            calculateCurrencyEntityList[changeLine-2] = currencyRepository.localDataSource.getCurrencyEntityByCurrencyType(changeCurrencyLive.value!!)

            /*val newCurrencyLine = _watchedCurrencyTypes.value?.get(changeLine-1)?.value?.let {
                CurrencyLine(changeCurrencyEntity,
                    it
                )
            }*/
            val currencyLines = mutableListOf<CurrencyLine>()
            ...
            for (_currencyEntity in calculateCurrencyEntityList){
                if(_currencyEntity.currency == _currencyEntity.baseCurrency){
                    val value = input / inputCurrencyEntity.rate
                    val currencyLine = CurrencyLine(_currencyEntity, value)

                    currencyLines.add(currencyLine)
                }else{
                    val value = input / inputCurrencyEntity.rate * _currencyEntity.rate
                    val currencyLine = CurrencyLine(_currencyEntity, value)
    ...
    fun toggleCurrencyFavorite(currencyModelEntity: CurrencyModelEntity){
        viewModelScope.launch {

            currencyModelEntity.isFavorite = !currencyModelEntity.isFavorite
            val updatedCurrencyModelId = currencyRepository.localDataSource.makeCurrencyFavorite(currencyModelEntity)

            Log.d("LOG_TAG","MainViewModel: makeCurrencyFavorite: ${currencyModelEntity.currencyType}, currencyModelId: $updatedCurrencyModelId")

            _newFavoriteCurrency.postValue(true)
        }
    }
    ...
    fun initCurrencies(calculateCurrencyTypes: List<CurrencyType>, input: Float, inputCurrencyType: CurrencyType){

        Log.d(TAG, "MainViewModel: initCurrencies: input: $input, inputCurrencyType: ${inputCurrencyType}, calculateCurrencyTypes: $calculateCurrencyTypes")
        ...
        viewModelScope.launch {
            ...
            val currencyEntities = currencyRepository.localDataSource.getCurrencyEntitiesByCurrencyTypes(calculateCurrencyTypes)

            calculateCurrencyEntityList = currencyEntities.toMutableList()
            ...
            for (_currencyEntity in currencyEntities){
                if(_currencyEntity.currency == _currencyEntity.baseCurrency){
                    val value = input / inputCurrencyEntity.rate
                    val currencyLine = CurrencyLine(_currencyEntity, value)
    ...
    //TODO: Only for worker
    fun downloadLatestRate(){
        viewModelScope.launch {
            ...
            var currencyEntityRub = CurrencyEntity(null, CurrencyType.RUB, CurrencyType.valueOf(latestCurrency.baseCurrency), latestCurrency.rates.rubRate)
            currencyRepository.localDataSource.insert(currencyEntityRub)

            Log.d(TAG,"MainViewModel: downloadLatestRate: rub rate: $currencyEntityRub")

            var currencyEntityUsd = CurrencyEntity(null, CurrencyType.USD, CurrencyType.valueOf(latestCurrency.baseCurrency), latestCurrency.rates.usdRate)
            currencyRepository.localDataSource.insert(currencyEntityUsd)

            Log.d(TAG,"MainViewModel: downloadLatestRate: usd rate: $currencyEntityUsd")
    ...
    fun getDBRates(){
        viewModelScope.launch {
            ...
            val currencyEntities = currencyRepository.localDataSource.getCurrenciesRateInfo()
            for (currencyEntity in currencyEntities){
                Log.d(TAG,"MainViewModel: getDBRates: $currencyEntity")
            }

        }
    }

    fun calculate(inputValue: Float){
        val currencyLines = mutableListOf<CurrencyLine>()

        Log.d(TAG,"MainViewModel: calculate: inputCurrency: $inputCurrencyEntity")
        this.inputValue = inputValue

        for(_currencyEntity in calculateCurrencyEntityList){

            //Log.d(TAG,"MainViewModel: calculate: inputValue: $inputValue, _currEntity: $_currencyEntity, inputCurrEntity: $inputCurrencyEntity")

            //Log.d(TAG,"MainViewModel: calculate: inputValue: $inputValue, _currEntity.rate: ${_currencyEntity.rate}, inputCurrEntity rate: ${inputCurrencyEntity.rate}")
            ...
            if(_currencyEntity.currency == _currencyEntity.baseCurrency){
                ...
                Log.d(TAG,"MainViewModel: calculate: value: $inputValue, inputCurrency: ${inputCurrencyEntity.currency}, result currencyLine: ${currencyLine.currencyEntity.currency}: ${currencyLine.value}")
                currencyLines.add(currencyLine)
            }

            //Convert to baseCurrency, then multiply on result currency rate
